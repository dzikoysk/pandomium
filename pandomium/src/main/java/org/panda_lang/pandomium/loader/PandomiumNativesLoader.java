...
public class PandomiumNativesLoader {
    ...
    protected void loadNatives(PandomiumLoader loader, boolean installUpdate, String url) throws Exception {
        ...
        if (checkNative(nativesDirectory) && !installUpdate) {
            return;
        }

        if (nativesDirectory.exists()) deleteDirectoryRecursively(nativesDirectory); // Remove old installation
        ...
        URL dependenciesURL = new URL(url);
        ...
        pandomium.getLogger().info("Downloading " + url);
